    ...
    function toolCardHtml(tool) {
        const url = tool.url || '';
        // tools.json에 favicon이 직접 지정돼 있으면(네이버 사다리처럼 구글의
        // 비공식 캐시에 그 페이지가 아예 없어서 인코딩을 아무리 고쳐도 안
        // 뜨는 경우를 위한 수동 지정) 구글을 거치지 않고 그 값을 그대로 쓴다.
        if (tool.favicon) {
            return `
            <a class="tool-card" href="${escapeHTML(url)}" target="_blank" rel="noopener">
                <span class="tool-card-ext">↗</span>
                <div class="tool-card-icon"><img loading="lazy" src="${escapeHTML(tool.favicon)}" alt="" onerror="this.style.display='none';"></div>
                <div class="tool-card-name">${escapeHTML(tool.name)}</div>
            </a>`;
        }
        // URL 안에 이미 퍼센트 인코딩된 문자(예: 한글 쿼리스트링)가 있으면 그대로
        // encodeURIComponent를 씌울 때 '%'가 '%25'로 다시 인코딩되면서(이중
        // 인코딩) 구글이 도메인을 못 알아본다. 인코딩을 한 번 풀었다가 다시
        // 인코딩하면 이미 인코딩돼 있던 부분과 그렇지 않은 부분이 전부 한 번만
        // 인코딩된 상태로 맞춰진다 - 인코딩된 문자가 애초에 없는 링크들은
        // 디코딩해도 원래 문자열 그대로라 동작이 전혀 안 바뀐다.
