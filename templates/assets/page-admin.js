...
function populateOffAirMembers() {
  const select=$('o_soop');const current=select.value;
  select.innerHTML='<option value="">멤버 선택</option>'+state.members.filter(m=>m.soop_id&&!m.left_date).map(m=>`<option value="${esc(m.soop_id)}">${esc(m.nickname)}</option>`).join('');
  select.value=current;
}
function attachMemberSuggestions(editor) {
  document.querySelectorAll('#adminMemberNames').forEach(el=>el.remove());
  const list=document.createElement('datalist');list.id='adminMemberNames';
  list.innerHTML=state.members.map(m=>`<option value="${esc(m.nickname)}"></option>`).join('');editor.append(list);
}
function attachVideoGroupSuggestions() {
  const input=$('vp_group');const list=document.createElement('datalist');list.id='adminVideoGroups';
  list.innerHTML=[...new Set(state.videoPicks.map(v=>v.group_name).filter(Boolean))].map(name=>`<option value="${esc(name)}"></option>`).join('');
  input.setAttribute('list',list.id);input.after(list);
  input.addEventListener('input',()=>{if(!$('vp_group_en').value){const match=state.videoPicks.find(v=>v.group_name===input.value);if(match)$('vp_group_en').value=match.group_en||'';}});
}
function renderHistoryMemberChips() {
  const members=state.members.map(m=>({'이름':m.nickname,'SOOP ID':m.soop_id}));
  $('adminHistoryChips').innerHTML=inputValue('h_members').split(/[,\r\n]+/).map(x=>x.trim()).filter(Boolean).map(name=>histMemberChipHtml(name,members,id=>`https://stimg.sooplive.com/LOGO/${encodeURIComponent(id.slice(0,2))}/${encodeURIComponent(id)}/m/${encodeURIComponent(id)}.webp`)).join('');
  const prefix=inputValue('h_members').split(',').slice(0,-1).join(',');
  const list=$('adminMemberNames');if(list)list.innerHTML=state.members.map(m=>`<option value="${esc(prefix?prefix+', '+m.nickname:m.nickname)}"></option>`).join('');
}
function closeEditSurface() {
  document.querySelectorAll('.admin-editor').forEach(el=>el.classList.add('hidden'));
}
async function openEditSurface(panel, edit) {
  ...
  if(['schedule','history'].includes(panel) && !state.members.length) await loadMembers();
  if(panel==='schedule') populateOffAirMembers();
  if(edit) await edit();
  if(panel==='schedule' && !edit) editSchedule();
  if(panel==='members' && !edit) editMember();
  if(panel==='history' && !edit) editHistory();
}
function connectPublicPage() {
  ...
  if(publicPageName==='schedule') {
    ...
    win.calOnDateSelect=date=>openEditSurface('schedule',()=>{calSelectedDateStr=date;$('o_date').value=date;editSchedule(null,date);});
