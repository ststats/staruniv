...
const state = {
  ...
  user: null,
  role: null,
  members: [], teams: [], matches: [], calendarEvents: [], offAir: [], navConfig: {}, historyEntries: [],
  tierPage: 0, tierCount: 0, tierPageSize: 100,
  currentMember: null, currentTeam: null, currentMatch: null, currentTier: null, currentSetting: null, currentHistory: null,
};
...
async function loadDashboard() {
  try {
    const tables = [['members','멤버'],['teams','팀'],['matches','팀 경기'],['tier_members','티어 선수'],['calendar_events','일정'],['calendar_off_air','휴방'],['elo_matches','ELO 경기'],['elo_players','ELO 선수']];
...
async function deleteSchedule(id){if(!confirm('이 일정을 삭제할까요?'))return;const {error}=await state.client.from('calendar_events').delete().eq('id',id);if(error)return setStatus(`삭제 실패: ${errText(error)}`,'error');await loadSchedule();}
...
async function deleteHistory(id){if(!confirm('이 연혁 메타데이터를 삭제할까요? 업로드 파일은 다른 항목이 참조할 수 있어 Storage에서 자동 삭제하지 않습니다.'))return;const {error}=await state.client.from('history_entries').delete().eq('id',id);if(error)return setStatus(`연혁 삭제 실패: ${errText(error)}`,'error');await loadHistoryEntries();}
