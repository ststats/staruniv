...
오래된 경기는 지금 실력을 덜 말해준다(반감기 6개월). 두 가중치를 곱해 경기마다 붙인다.
...
# 반감기를 정할 때 시계열 홀드아웃으로 재봤다(컷오프 4개 × 이후 90일, 로그손실).
# 365일 0.6723 · 270일 0.6716 · 180일 0.6703 · 120일 0.6691 · 90일 0.6688 · 감쇠없음 0.6784.
# 짧을수록 잘 맞히지만 표본이 얇아져 표준오차가 커진다(365일 0.195 → 180일 0.254 →
# 90일 0.316). 180일이 개선분의 절반 이상을 가져오면서 순위가 덜 출렁이는 자리다.
# 티어표 자체가 1~2달마다 갱신되니, 그보다 훨씬 긴 창으로 보면 이미 티어에 반영된
# 오래된 경기를 다시 세게 된다.
HALF_LIFE_DAYS = 180.0      # 최근 가중치 반감기(6개월)
