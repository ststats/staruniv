...
#profile-live-embed iframe { display: block; width: 100%; aspect-ratio: 16 / 9; border: 0; border-radius: 10px; }
...
(function () {{
  ...
  function renderProfile(data) {{
      ...
      // 방송 중이면 그 사람 프로필에 실제 방송 화면을 띄운다. play.sooplive.co.kr/{{아이디}}로
      // 들어가면 방송번호(broad_no) 없이도 그 사람의 "지금 하고 있는 방송"이 자동으로 잡혀서
      // iframe으로 재생된다(실제 브라우저에서 확인함). checkIsLive는 위쪽 라이브 점 표시
      // 기능이랑 똑같은 함수를 그대로 재사용한다 - 같은 API/캐시를 공유.
      const liveEmbedEl = document.getElementById('profile-live-embed');
      if (tid) {{
          checkIsLive(tid).then(isLive => {{
              if (isLive) {{
                  liveEmbedEl.innerHTML = `<iframe src="https://play.sooplive.co.kr/${{tid}}" allowfullscreen></iframe>`;
