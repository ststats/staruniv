...
/* Grid 구조에서 minmax(0, 1fr)로 요소 팽창을 완벽 차단. 자식들은 무조건 폭 100% 안에서 정렬됨 */
.profile-live-info { display: grid; grid-template-columns: minmax(0, 1fr); grid-template-rows: repeat(3, 1fr); align-items: center; min-width: 0; flex: 1; padding: 2px 0; }
.profile-live-title { font-size: 13px; font-weight: 700; color: #1a1d29; width: 100%; text-align: right; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.profile-live-viewer, .profile-live-elapsed { font-size: 12px; font-weight: 600; color: #6b6f79; width: 100%; text-align: right; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
          ...
          targetTeamStats.forEach(ts => {{
              ...
              const makeRows = (list, padLen) => {{
                  let rHtml = list.map(m => {{
                      let cClass = [];
                      // ★ 수정됨: 수장/전력외는 값이 0이어도 무조건 붉은색(excluded) 처리
