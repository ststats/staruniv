...
/* 
  ...
  3) 그 버그의 표준 우회법이 direction:rtl 트릭이다 - 컨테이너를 RTL로 두면
     잘림/정렬 계산 기준이 바뀌면서 오른쪽 끝에 정확히 붙는다. 대신 원본
     텍스트가 한글/영문 섞여도 글자 순서가 이상하게 재배열되면 안 되므로,
     unicode-bidi: plaintext를 같이 줘서 실제 표시 순서는 텍스트 내용
     자체(유니코드 양방향 알고리즘)로 판단하게 한다 - 컨테이너의 강제 RTL
     방향은 잘림 계산에만 영향을 주고, 글자 순서 자체는 안 바뀐다.
...
.profile-live-title { width: 100%; direction: rtl; unicode-bidi: plaintext; text-align: left; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; padding-right: 4px; font-size: 13px; font-weight: 700; color: #1a1d29; line-height: 1.3; }
.profile-live-viewer, .profile-live-elapsed { width: 100%; direction: rtl; unicode-bidi: plaintext; text-align: left; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; padding-right: 2px; font-size: 12px; font-weight: 600; color: #6b6f79; line-height: 1.3; }
