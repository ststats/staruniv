...
-- 영상(mp4·webm)은 방송통계 TOP 대표 사진용(움짤보다 가볍고 부드럽다). quicktime(아이폰 .mov)은 서버 편집 전 원본용.
insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values ('staruniv-media','staruniv-media',true,10485760,array['image/jpeg','image/png','image/webp','image/gif','video/mp4','video/webm','video/quicktime'])
...
-- ############################################################################
-- 10. 멤버 대표 영상 서버 편집(모바일)
-- ############################################################################

-- PC 브라우저는 올릴 때 직접 줄인다. 못 하는 기기(모바일)는 원본을 members-photo-raw/에 올리고 여기 작업을 넣으면
-- GitHub Actions(member-video.yml)가 ffmpeg로 줄여 members.photo_path에 등록한다(scripts/member_video.py).
create table if not exists public.media_jobs (
  ...
  result_path text,
  error text,
  dispatch_request bigint,
  created_at timestamptz not null default now(),
  started_at timestamptz,
  finished_at timestamptz
);
alter table public.media_jobs enable row level security;
drop policy if exists media_jobs_admin_read on public.media_jobs;
create policy media_jobs_admin_read on public.media_jobs for select to authenticated using ((select public.is_admin()));
revoke all on public.media_jobs from anon, authenticated;
grant select on public.media_jobs to authenticated;

create or replace function public.admin_request_member_video(p_member_id bigint, p_raw_path text)
returns bigint
language plpgsql
security definer
set search_path = public, extensions
...
begin
  ...
  if p_raw_path is null or p_raw_path !~ '^members-photo-raw/[A-Za-z0-9_.-]+$' or p_raw_path like '%..%' then
    raise exception '원본 영상 경로가 올바르지 않습니다.';
  end if;
  if not exists (select 1 from public.members where id = p_member_id) then
    raise exception '멤버 %이 없습니다.', p_member_id;
  end if;
  select decrypted_secret into token
  from vault.decrypted_secrets where name = 'github_actions_token' limit 1;
  if token is null or token = '' then
  ...
  insert into public.media_jobs (member_id, raw_path) values (p_member_id, p_raw_path) returning id into job_id;
  select net.http_post(
    url := 'https://api.github.com/repos/ststats/staruniv/actions/workflows/member-video.yml/dispatches',
    body := jsonb_build_object('ref', 'main', 'inputs', jsonb_build_object('job_id', job_id::text)),
    headers := jsonb_build_object(
      'Authorization', 'Bearer ' || token,
      'Accept', 'application/vnd.github+json',
  ...
  update public.media_jobs set dispatch_request = req where id = job_id;
  return job_id;
end;
$$;
revoke all on function public.admin_request_member_video(bigint, text) from public, anon;
grant execute on function public.admin_request_member_video(bigint, text) to authenticated;
